...
class History extends Component {
    ...
    onBtnDetailClick = (trxId) => {
        console.log(trxId)
        axios.get(API_URL + `/transaction/historydetail/${trxId}` 
           )
        ...
        var listJSXTransaksi = this.state.transactionList.map((item) => {
            return (
                <tr key={item.trxId}>
                    ...
                    <td>{item.totalQty}</td>
                    <td>{item.status}</td>
                    <td> <button type="submit" className="btn btn-primary" style={{ fontSize: "12px" }}
                    onClick={() => this.onBtnDetailClick(item.trxId)}>Detail</button></td>
